...
var blog_post_formatter = function (blogposts) {
  var formatted =
    '<div class="focus_grid-container"><section class="focus_grid-inner">'
  for (var i = 0; i < blogposts.length; i++) {
    var blogpost = blogposts[i]

    var the_date = new Date(blogpost.publishDate)
    var day = the_date.toLocaleDateString(current_lang, { day: 'numeric' })
    var month_short = the_date.toLocaleDateString(current_lang, {
      month: 'short'
    })
    ...
    formatted +=
      '<article class="focus_grid-col focus_grid--' +
      cols +
      ' ' +
      states[inc] +
      '">'
    ...
    formatted +=
      '<time class="focus_widget_post_item-date">' +
      final_date_output +
      '</time>'
    formatted += '</div></a></div>'
    formatted += '</article>'
